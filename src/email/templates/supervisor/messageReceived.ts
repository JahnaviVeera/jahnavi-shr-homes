export function supervisorMessageReceivedEmail(data: {
    ...
    const roleLabel = data.senderRole === 'admin' ? 'Admin' : 'Customer';
    return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f9fafb;padding:24px;border-radius:12px;">
      <h2 style="color:#7c3aed;">💬 New Message from ${roleLabel}</h2>
      <p>Hi ${data.supervisorName},</p>
      <p><strong>${data.senderName}</strong> (${roleLabel}) has sent you a new message:</p>
      ...
      <a href="${data.frontendUrl}/supervisor/messages"
