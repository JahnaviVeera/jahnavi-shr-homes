...
router.get("/budget-summary", authenticate, authorizeRoles("admin"), paymentController.getBudgetSummary); // Budget summary probably admin only? Or allow supervisor/user? Prompt says "Payments" creation is admin only. Viewing is usually open. Let's start with all authenticated for View, except general summary which might be sensitive.
